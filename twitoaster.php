...
/*
Plugin Name: Twitter Comments - Twitoaster
Plugin URI: http://twitoaster.com/wordpress-plugin/
Description: Twitter Comments system. Automatically retrieve Twitter Replies and/or Twitter Retweets to your Blog's Posts using WP built-in comment system. Also bring Twitter Statistics.
Version: 1.3.7
Author: Twitoaster
...
//------------------------------------------------------------------------------
	...
	$twitoaster_meta = array();
        ...
        // Twitoaster Deprecation Notice
        function twitoaster_deprecation_notice() {
                echo '<div id="message" class="updated fade"><p><strong>Twitoaster Plugin is deprecated! <a href="http://blog.twitoaster.com/twitoaster-shutting-down">Please read this announcement for more information</a></strong></p></div>';
        }
        add_action( 'admin_notices', 'twitoaster_deprecation_notice' );
...
//------------------------------------------------------------------------------
	...
	function twitoaster_options($options = array(), $reset = false) {
		...
		$currentoptions['install_version'] = '1.3.7';
		...
		$currentoptions['post_replies']['auto_threads_shortener'] = ((!(isset($options['post_replies']['auto_threads_shortener']))) OR (empty($options['post_replies']['auto_threads_shortener']))) ? 'bit.ly' : $options['post_replies']['auto_threads_shortener'];
		...
		$currentoptions['post_replies']['show_retweets'] = (!(isset($options['post_replies']['show_retweets']))) ? 'yes' : $options['post_replies']['show_retweets'];
...
//------------------------------------------------------------------------------
// Twitoaster / Twitter API Function
//------------------------------------------------------------------------------
	...
	function twitoaster_api_process($process, $http_error)
	{
		...
		if (!(is_serialized($process))) {
			...
			else { $result['error']['message'] = '<strong>Error: Connection problem, please check your Web hosting provider (unable to reach api.twitoaster.com on port 80 from your installation)</strong>'; }
		}
...
//------------------------------------------------------------------------------
// Twitoaster / Twitter Config (Admin Page)
//------------------------------------------------------------------------------
	...
	function twitoaster_options_page()
	{
		...
		// Post Replies Form
			...
			if (isset($_POST['twitoaster-post-replies-auto-threads-shortener'])) {
				if (!(empty($_POST['twitoaster-post-replies-auto-threads-shortener']))) { $newtwitoaster_options['post_replies']['auto_threads_shortener'] = stripslashes($_POST['twitoaster-post-replies-auto-threads-shortener']); }
				else { $newtwitoaster_options['post_replies']['auto_threads_shortener'] = ''; }
			}
			...
			if (isset($_POST['twitoaster-post-replies-show-retweets'])) {
				if ($_POST['twitoaster-post-replies-show-retweets'] == 'yes') { $newtwitoaster_options['post_replies']['show_retweets'] = 'yes'; }
				else { $newtwitoaster_options['post_replies']['show_retweets'] = 'no'; }
			}

            $message = 'Twitoaster Comments settings successfully Updated.';
		}
		...
		?>
			...
			<div class="wrap">
				...
				<p>
					...
					<br />Feel free to <b><a href="http://twitoaster.com/about/contact/" target="_blank">contact us</a></b> if you experience any trouble with this plugin or your Twitoaster account, and think to <b>follow <a href="http://twitter.com/twitoaster" target="_blank">@twitoaster</a></b> to be informed of the latest Twitter + WordPress news!
				</p>
				...
				<h2>Twitter Comments (posts replies &amp; retweets)</h2>
				<h3>Twitter Comments Settings</h3>
				<p>
					Automatically retrieve Twitter replies and/or retweets to your Blog's posts. These "Twitter comments" are displayed like comments, on the posts pages they are related to.
					<br /><strong>Example:</strong> You Post an article on your blog; You Tweet it (or let the plugin do it for you); You automatically get all related Twitter replies and/or retweets as comments on your Blog post.
				</p>
				<form name="twitoaster-form-post-replies" method="post" action="#">
					<table class="form-table">
						...
						<tr valign="top">
							...
							<td>
								<fieldset>
									...
									<br />
									<label>
										URL shortener:&nbsp;<select name="twitoaster-post-replies-auto-threads-shortener" id="twitoaster-post-replies-auto-threads-shortener"<?php if (!($currentoptions['post_replies']['twitter_comments'] == 'yes')) { echo ' disabled="disabled"'; }?>>
											<option value="bit.ly"<?php selected($currentoptions['post_replies']['auto_threads_shortener'], 'bit.ly'); ?>><?php _e('bit.ly'); ?></option>
											<option value="toast.tw"<?php selected($currentoptions['post_replies']['auto_threads_shortener'], 'toast.tw'); ?>><?php _e('toast.tw'); ?></option>
										</select>
						...
						<tr valign="top">
							...
							<td>
								<fieldset>
									<legend class="hidden">Retweets</legend>
									<label title="yes">
										<input name="twitoaster-post-replies-show-retweets" value="yes" type="radio"<?php checked($currentoptions['post_replies']['show_retweets'], 'yes'); ?><?php if (!($currentoptions['post_replies']['twitter_comments'] == 'yes')) { echo ' disabled="disabled"'; }?> />&nbsp;Display Replies and Retweets (RT).
										<span class="description">&nbsp;<?php if (!($currentoptions['post_replies']['twitter_comments'] == 'yes')) { ?>Enable Twitter Comments to use this option.<?php } ?></span>
									</label>
									<br />
									<label title="no">
										<input name="twitoaster-post-replies-show-retweets" value="no" type="radio"<?php checked($currentoptions['post_replies']['show_retweets'], 'no'); ?><?php if (!($currentoptions['post_replies']['twitter_comments'] == 'yes')) { echo ' disabled="disabled"'; }?> />&nbsp;Do not display Retweets.
										<span class="description">&nbsp;<?php if (!($currentoptions['post_replies']['twitter_comments'] == 'yes')) { ?>Enable Twitter Comments to use this option.<?php } ?></span>
									</label>
								</fieldset>
							</td>
						...
						<tr valign="top">
							...
							<td>
								<fieldset>
									...
									<label title="yes">
										<input name="twitoaster-post-replies-show-threads" value="yes" type="radio"<?php checked($currentoptions['post_replies']['show_threads'], 'yes'); ?><?php if (!($currentoptions['post_replies']['twitter_comments'] == 'yes')) { echo ' disabled="disabled"'; }?> />&nbsp;Display your Tweets and their replies.
										<span class="description">&nbsp;<?php if (!($currentoptions['post_replies']['twitter_comments'] == 'yes')) { ?>Enable Twitter Comments to use this option.<?php } ?></span>
									</label>
									<br />
									<label title="no">
										<input name="twitoaster-post-replies-show-threads" value="no" type="radio"<?php checked($currentoptions['post_replies']['show_threads'], 'no'); ?><?php if (!($currentoptions['post_replies']['twitter_comments'] == 'yes')) { echo ' disabled="disabled"'; }?> />&nbsp;Do not display your own Tweets.
										<span class="description">&nbsp;<?php if (!($currentoptions['post_replies']['twitter_comments'] == 'yes')) { ?>Enable Twitter Comments to use this option.<?php } ?></span>
									</label>
								</fieldset>
							</td>
				...
				<p>
					<strong>Warning</strong>: this will delete all your Twitter comments! You might want to do it if you changed your Twitter account, or your Twitter Comments Display options.
				...
				<h2>Twitter Widgets (charts &amp; conversations)</h2>
				<h3>Twitter Charts</h3>
				<p>
					Analytics charts showing how many replies / retweets you are generating, and what day of the week (or time of day) produces the most replies and retweets.
				...
				<h3>Twitter Conversations</h3>
				<p>
					Listing of your 20 most recent Twitter conversations (groups your Twitter replies and retweets with the Tweets that inspired them).
