...
//------------------------------------------------------------------------------
// Twitoaster / Twitter post replies (Twitter Comments)
//------------------------------------------------------------------------------

	function twitoaster_post_replies_twitter_comments($post)
	{
		global $twitoaster_options, $twitoaster_meta;
		...
		if (($twitoaster_meta['expire'] < current_time('timestamp', true)) AND (comments_open($post->ID)) AND (get_post_time('G', true, $post)) AND (get_post_time('G', true, $post) > 0) AND (get_post_time('G', true, $post) < (current_time('timestamp', true) - 60)) AND ((empty($twitoaster_meta['working'])) OR ($twitoaster_meta['working'] < (current_time('timestamp', true) - 60))))
		{
			...
			$twitoaster_api_result = twitoaster_api_request('GET', 'conversation/search', array('query' => get_permalink($post->ID), 'user_id' => $twitoaster_options['user_id'], 'in_urls_only' => true, 'in_titles_only' => true, 'show_replies' => true));
			...
			// Twitoaster API Error Handling
		 	if ($twitoaster_api_result['error']) {
				if ($twitoaster_api_result['error']['user_issue']) {
					...
					$twitoaster_meta['expire'] = (current_time('timestamp', true) + $twitoaster_options['api_timing']['instant']);
				}
			}
			...
			// Twitoaster API Data Processing
				...
				if (get_post_time('G', true, $post) > (current_time('timestamp', true) - 3600)) { $expire = $twitoaster_options['api_timing']['instant']; } /* < 1h */
				else if (get_post_time('G', true, $post) > (current_time('timestamp', true) - 21600)) { $expire = $twitoaster_options['api_timing']['short']; } /* < 1h - 6h */
				else if (get_post_time('G', true, $post) > (current_time('timestamp', true) - 172800)) { $expire = $twitoaster_options['api_timing']['normal']; } /* 6h - 48h */
				...
				$twitoaster_meta['expire'] = (current_time('timestamp', true) + $expire);
				...
				foreach ($twitoaster_api_result['data'] AS $conversation) {
					...
					{
						// Twitter Comment Insert?
						if (!(array_key_exists($conversation->thread->id, $twitoaster_meta['content']))) {
							if ($twitoaster_meta_comment = twitoaster_post_replies_comment_insert($post, 0, $conversation->thread, false)) {
								$twitoaster_meta['content'][$conversation->thread->id] = $twitoaster_meta_comment;
								twitoaster_post_replies_update_meta('_twitoaster_post_replies', $post->ID, $twitoaster_meta);
								$comment_parent_default = $twitoaster_meta_comment['comment_id'];
							}
						}

						// Twitter Comment Update?
						else {
							$comment_parent_default = $twitoaster_meta['content'][$conversation->thread->id]['comment_id'];
							if ($twitoaster_meta_comment = twitoaster_post_replies_comment_update($post, 0, $conversation->thread, $twitoaster_meta['content'][$conversation->thread->id])) {
								$twitoaster_meta['content'][$conversation->thread->id] = $twitoaster_meta_comment;
								twitoaster_post_replies_update_meta('_twitoaster_post_replies', $post->ID, $twitoaster_meta);
							}
						}
					...
					if (isset($conversation->replies)) {
						foreach($conversation->replies AS $reply) {
							if (($reply->type == 'reply') OR (($reply->type == 'retweet') AND ($twitoaster_options['post_replies']['show_retweets'] == 'yes')))
							{
								// Twitter Comment Insert?
								if (!(array_key_exists($reply->id, $twitoaster_meta['content']))) {
									if (array_key_exists($reply->in_reply_to_status_id, $twitoaster_meta['content'])) { $comment_parent = $twitoaster_meta['content'][$reply->in_reply_to_status_id]['comment_id']; }
									else { $comment_parent = $comment_parent_default; }
									if ($twitoaster_meta_comment = twitoaster_post_replies_comment_insert($post, $comment_parent, $reply, false)) {
										$twitoaster_meta['content'][$reply->id] = $twitoaster_meta_comment;
										twitoaster_post_replies_update_meta('_twitoaster_post_replies', $post->ID, $twitoaster_meta);
									}
								}
								...
								// Twitter Comment Update?
									...
									if ($twitoaster_meta_comment = twitoaster_post_replies_comment_update($post, $comment_parent, $reply, $twitoaster_meta['content'][$reply->id])) {
										$twitoaster_meta['content'][$reply->id] = $twitoaster_meta_comment;
										twitoaster_post_replies_update_meta('_twitoaster_post_replies', $post->ID, $twitoaster_meta);
									}
								}
			...
			// Twitoaster Meta UN-Lock
			$twitoaster_meta['working'] = false;
			twitoaster_post_replies_update_meta('_twitoaster_post_replies', $post->ID, $twitoaster_meta);
		}
	}

	function twitoaster_post_replies_comment_insert($post, $comment_parent, $twitoaster_data, $comment_id = false) {
		global $twitoaster_options;

		// Comment's data preparation
		$data = array(
		...
		if ($comment_id) {
			if ($comment = get_comment($comment_id)) {
				...
				$comment_update = wp_update_comment($data);
		...
		// Twitter Insert
		else if (($comment_id = wp_insert_comment($data)) AND (is_numeric($comment_id)) AND ($comment_id > 0)) {
	...
	function twitoaster_post_replies_comment_update($post, $comment_parent, $twitoaster_data, $twitoaster_meta_comment) {
		if ($comment = get_comment($twitoaster_meta_comment['comment_id'])) {
			$comment_author_url = ($twitoaster_data->user->url == false) ? '' : $twitoaster_data->user->url;

			if (($twitoaster_data->user->screen_name != $comment->comment_author) OR ($comment_parent != $comment->comment_parent) OR ($comment_author_url != $comment->comment_author_url) OR ($twitoaster_data->user->profile_image_url != $twitoaster_meta_comment['avatar']) OR ($twitoaster_data->url != $twitoaster_meta_comment['url'])) {
				return twitoaster_post_replies_comment_insert($post, $comment_parent, $twitoaster_data, $twitoaster_meta_comment['comment_id']);
			}
			else { return false; }
		}
		else { return false; }
	...
	function twitoaster_post_replies_get_meta($meta_key, $post_ID, $manual = false) {
		if ((isset($post_ID)) AND (is_numeric($post_ID)) AND ($post_ID > 0)) {
			if (!($manual)) {
				// Retrieving the meta with WP functions (wp-cache might be enabled)
				$meta = get_post_meta($post_ID, $meta_key, true);
			}
			else {
				// Retrieving the meta manually (bypassing any wp-cache)
				global $wpdb;
				$meta = $wpdb->get_var($wpdb->prepare("SELECT meta_value FROM $wpdb->postmeta WHERE post_id = %d AND meta_key = %s", $post_ID, $meta_key));
	...
	function twitoaster_post_replies_comment_avatar($avatar, $comment = false, $size = '32', $default = '', $alt = false) {
		if ($comment == 'wp-plugin@twitoaster.com') { global $comment; }
	...
	function twitoaster_post_replies() {
		...
		if (is_single()) {
			...
			if (($twitoaster_options['api_key'] != '') AND ($twitoaster_options['post_replies']['twitter_comments'] == 'yes')) {
				twitoaster_post_replies_twitter_comments($post);
			}
		}
	}
...
//------------------------------------------------------------------------------
// Twitoaster / Twitter post replies Auto Threading (Tweet Post)
//------------------------------------------------------------------------------
	...
	function twitoaster_post_replies_auto_thread($post_ID)
	{
		...
		else if ((isset($post_ID->ID)) AND (is_numeric($post_ID->ID))) { $post = get_post($post_ID->ID); }
		$twitoaster_meta_thread = twitoaster_post_replies_get_meta('_twitoaster_post_auto_thread', $post->ID);

		// Needs Auto Threading
		if (($twitoaster_options['post_replies']['twitter_comments'] == 'yes') AND (empty($twitoaster_meta_thread['content'])) AND ($twitoaster_meta_thread['post_replies']['auto_threads'] == 'yes') AND ((empty($twitoaster_meta_thread['working'])) OR ($twitoaster_meta_thread['working'] < (current_time('timestamp', true) - 60))))
		{
			...
			// Twitoaster API Request
			$twitoaster_api_result = twitoaster_api_request('POST', 'user/update', array('api_key' => $twitoaster_options['api_key'], 'extended' => false, 'status' => $status, 'shortener' => $twitoaster_options['post_replies']['auto_threads_shortener']), 20);
	...
	add_action('future_to_publish', 'twitoaster_post_replies_auto_thread');
